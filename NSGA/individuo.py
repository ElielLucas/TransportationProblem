...
class Individuo:
    ...
    def montar_solução_random(self):
        ...
        for i, demanda in enumerate(self.demandas_clientes):
            ...
            alocacao_produtores = (
                self.distribuir_demanda_aleatoriamente_para_produtores(
                    demanda=demanda,
                    ofertas=self.ofert_prod,
                    quantidade_produtores=len(self.produtores),
                )
            )
            self.distribuir_demanda_de_produtores_para_transbordo_e_portos(
                gene_origem=alocacao_produtores,
                gene_intermediario=alocacao_transbordos,
                gene_destino=alocacao_portos,
                capacidade_intermediarios=self.cap_trans,
                capacidade_destino=self.cap_destino,
                pontos_sem_capacidade=pontos_sem_capacidade,
                origens=self.produtores,
                intermediarios=self.ferrovias,
                destinos=self.portos,
                cromossomo=self.cromossomos[i],
            )
            self.distribuir_demanda_de_transbordos_para_portos(
                gene_intermediario=alocacao_transbordos,
                gene_destino=alocacao_portos,
                capacidade_destino=self.cap_destino,
                pontos_sem_capacidade=pontos_sem_capacidade,
            ...
            self.cromossomos[i].set_genes(
                gene_produtores=alocacao_produtores.copy(),
                gene_transbordos=alocacao_transbordos.copy(),
                gene_portos=alocacao_portos.copy(),
            )

    def distribuir_demanda_aleatoriamente_para_produtores(
        self, demanda, ofertas, quantidade_produtores
    ):
        vetor = np.zeros(quantidade_produtores)
        while demanda > 0:
            produtor = random.choice(np.where(ofertas - vetor > 0)[0])
            aleatorio = random.randint(
                0, int(min(ofertas[produtor] - vetor[int(produtor)], demanda))
            )
            vetor[produtor] += aleatorio
            demanda -= aleatorio
        return vetor

    def distribuir_demanda_de_produtores_para_transbordo_e_portos(
        self,
        gene_origem,
        gene_intermediario,
        gene_destino,
        capacidade_intermediarios,
        capacidade_destino,
        ...
        for produtor in origens:
            ...
            while alocacao_a_ser_distribuida > 0:
                ponto_mais_proximo = self.find_nearest_neighbor(
                    ponto_referencia=produtor,
                    possiveis_destinos=intermediarios + destinos,
                    pontos_sem_capacidade=pontos_sem_capacidade,
                )
                if ponto_mais_proximo in intermediarios:
                    demanda_alocar = min(
                        alocacao_a_ser_distribuida,
                        capacidade_intermediarios[ponto_mais_proximo - inp.range_trans],
                    )
                    if demanda_alocar > 0:
                        gene_intermediario, alocacao_a_ser_distribuida = (
                            self.alocar_demanda(
                                provider=produtor,
                    ...
                    demanda_alocar = min(
                        alocacao_a_ser_distribuida,
                        capacidade_destino[ponto_mais_proximo - inp.range_port],
                    )
                    if demanda_alocar > 0:
                        gene_destino, alocacao_a_ser_distribuida = self.alocar_demanda(
                            provider=produtor,
                            target=ponto_mais_proximo,
    ...
    def distribuir_demanda_de_transbordos_para_portos(
        self,
        gene_intermediario,
        gene_destino,
        capacidade_destino,
        ...
        for transbordo in intermediarios:
            if aloc_trans[transbordo - inp.range_trans] > 0:
                ...
                while alocacao_a_ser_distribuida > 0:
                    ponto_mais_proximo = self.find_nearest_neighbor(
                        ponto_referencia=transbordo,
                        possiveis_destinos=destinos,
                        pontos_sem_capacidade=pontos_sem_capacidade,
                    )
                    gene_destino, alocacao_a_ser_distribuida = self.alocar_demanda(
                        provider=transbordo,
                        target=ponto_mais_proximo,
                        range_point=[inp.range_trans, inp.range_port],
                        provider_allocation=aloc_trans,
                        target_point_allocation=gene_destino,
                        target_point_capacity=capacidade_destino,
                        total_demanda=alocacao_a_ser_distribuida,
                        cromossomo=cromossomo,
    ...
    def objective_function(self):
        ...
        for i in range(len(self.demandas_clientes)):
            custo, emissao = self.calcular_custo_e_emissao_transporte(
                gene=self.cromossomos[i]
            )
            custo_total += custo
    ...
    def find_nearest_neighbor(
        self, ponto_referencia, possiveis_destinos, pontos_sem_capacidade
    ):
        ...
        for i, distancia in enumerate(inp.dist_matrix[ponto_referencia]):
            if (
                i in possiveis_destinos
                and i not in pontos_sem_capacidade
                and distancia < vizinho_mais_proximo
            ):
    ...
    def calcular_custo_e_emissao_transporte(self, gene):
        ...
        for node_ini, aresta in gene.lista_adjacencia.items():
            for node_fim, qtd_transportada in aresta.items():
                if (node_ini in N and node_fim in K) or (
                    node_ini in N and node_fim in M
                ):
                    custo_modal = inp.cr
                ...
                custo_transporte += (
                    qtd_transportada * inp.dist_matrix[node_ini][node_fim] * custo_modal
                )
                emissao_transporte += (
                    qtd_transportada
    ...
    def alocar_demanda(
        ...
        range_point,
        provider_allocation,
        target_point_allocation,
        target_point_capacity,
        total_demanda,
        cromossomo,
    ):
        ...
        new_allocation = min(
            provider_allocation[provider_index], target_point_capacity[target_index]
        )
