...
# for i in N:
#     m.addConstr(gp.quicksum(X[i, j, o] for j in M for o in O) <= oferta[i], "Oferta_Prod_{}".format(i))
...
for j in M:
    m.addConstr(((gp.quicksum(X[i, j, o] for j in M for o in O)) + (gp.quicksum(X[k, j, o] for k in K for o in O))) <= CP[j], "Cap_Porto_{}".format(j))
